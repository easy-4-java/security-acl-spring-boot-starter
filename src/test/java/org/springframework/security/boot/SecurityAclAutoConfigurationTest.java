...
class SecurityAclAutoConfigurationTest {
    ...
    @Test
    @DisplayName("Auto-configuration loads when 'spring.security.acl.enabled=true'")
    void testLoadsWhenEnabledPropertySet() {
        runner.withUserConfiguration(SecurityAclAutoConfiguration.class)
                .withPropertyValues("spring.security.acl.enabled=true")
                .run(context -> assertThat(context).hasSingleBean(SecurityAclAutoConfiguration.class));
    }
    ...
    void testNotLoadedWhenPropertyAbsent() {
        runner.withUserConfiguration(SecurityAclAutoConfiguration.class)
                .run(context -> assertThat(context).doesNotHaveBean(SecurityAclAutoConfiguration.class));
    }
}
